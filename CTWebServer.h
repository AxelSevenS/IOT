...
void handleConfig(AsyncWebServerRequest *request) {
    ...
    "<html lang='fr'>"
      ...
      "<body>"
        ...
        "<form action='/monitoringConfigSubmit' method='POST'>"
          ...
          "<br>"
          "<input type='number' step='any' min='0' name='temperature' placeholder='Temperature Seuil de Maladie' value='{TEMPERATURE_THRESHOLD}'>"
          "<label for='temperature'>Temperature Seuil de Maladie (en °C)</label>"
          "<br>"
          "<input type='submit' value='Enregistrer'>"
        "</form>"
  ...
  htmlPage.replace("{TEMPERATURE_THRESHOLD}", String(temperature_threshold));
...
void handleMonitoringConfigSubmit(AsyncWebServerRequest *request) {
  ...
  float temperature = atof(request->arg("temperature").c_str());

  update_monitoring_config(time, distance, temperature);
