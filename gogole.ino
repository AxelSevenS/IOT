...
* \section Définition :
    ...
    * STATION (WIFI_STA ) : 
        ...
        L’ESP32 se comporte comme un ordinateur qui serait connecté à notre box. Si la box est reliée à Internet,
        alors l’ESP32 peut accéder à Internet. L’ESP32 peut se comporter en tant que client ,
        c’est-à-dire faire des requêtes aux autres appareils connectés sur le réseau ou en tant que serveur ,
        c’est-à-dire que d’autres appareils connectés sur le réseau vont envoyer des requêtes à l’ESP32. Dans les 2 cas,
        l’ESP32 peut accéder à Internet.
    ...
    * AP (Access Point) (WIFI_AP ) : 
        ...
        La latence et le débit seront moins bons qu’avec une box classique.
...
* \section Matériels:
    ...
    - Un téléphone (Hamburger = ACCESS POINT) nous sert de point d'accés internet.


* \section Etapes :

 * \subsection Etape 1 : Mise en place d'une connection entre les esp (En mode STATION) et le point d'accées (Hamburger) :
 
        - Utilisation de la librairie MyWiFi.h (permets la connexion simultanée de plusieurs appareils sur un même access point).
        Function Utiliséee :
            - setup_Wifi():
                Va permettre de setup l'access point et vérfier si l'utilisateur en mode station peut ce connecter.
            - connect_WiFi_AP(const char* ssid, const char* password) et check_WiFi_connection():
...
* \subsection Etape 2 : Mise en place d'un serveur web (Protocole en MQTT : PUB / SUB) :

        - Utilisation de la librairie MyWiFiServer.h (permet la mise en place d'un server web en local, qui permet de recevoir les informations sur un autre utilisateurs si abonnée).

 * \subsection Etape 3 : Mise en place du dashboard via Adafruit (Protocole en MQTT : PUB / SUB)
...
* \subsection Etape 4 :Mise en place, utilisation et test des statuts des utilisateurs unique des esps 
