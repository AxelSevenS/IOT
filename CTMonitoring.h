...
/**
 * @brief Vérifie si l'appareil est marqué comme malade dans le fichier de monitoring et met à jour le status de l'utilisateur si c'est le cas
 * 
...
/**
 * @brief Met à jour le status d'un appareil dans le fichier de monitoring
 * 
 * @param address L'adresse Bluetooth de l'appareil à mettre à jour
 * @param status Le nouveau status de l'appareil (Sain, Cas contact, Malade)
 * 
*/
void update_device_status(const char* address, const char* status) {
  ...
  read_write_file(MONITORING_FILE_PATH, [=](JsonDocument& monitoringDocument, File& monitoringFile) {
    ...
    MYDEBUG_PRINT("-Appareil : Mis à jour le statut de ");
...
/**
 * @brief Effectue une "interaction" avec un appareil, c'est à dire met à jour son nom, son temps d'exposition et son statut dans le fichier de surveillance\n
 * Si le temps d'exposition est supérieur au seuil de surveillance, l'appareil est marqué pour surveillance rapprochée.
 * 
 * @param address L'adresse Bluetooth de l'appareil à mettre à jour
 * @param name Le nom de l'appareil à mettre à jour (uniquement visuel)
 * @param last_contact Le temps de la dernière interaction avec l'appareil
 * 
*/
void device_interact(const char* address, const char* name, long last_contact) {
  
  MYDEBUG_PRINT("-Appareil : Intéragi avec ");
