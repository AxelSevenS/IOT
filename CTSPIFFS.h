 ...
 * En utilisant ce module, il est possible de modifier les différentes variables globales système :
 * - @ref device_name : Le nom de l'appareil
 * - @ref device_password : Le mot de passe de l'appareil
 * - @ref net_ssid : Le nom du réseau WiFi
 * - @ref net_password : Le mot de passe du réseau WiFi
 * - @ref monitoring_time : Le temps de surveillance
 * - @ref monitoring_distance : La distance de surveillance
 * - @ref user_status : L'état de santé de l'utilisateur
 * - @ref temperature_threshold : Le seuil de température
 * 
 ...
 * Les contenus du fichier sont passés à la fonction writeFunction sous forme de JsonDocument.\n
 * 
 * @param new_device_name Le nouveau nom de l'appareil.
 * @param new_device_password Le nouveau mot de passe de l'appareil.
 * @param jsonDocument Le document JSON à écrire dans le fichier de configuration.
 */
void update_device_config( const char* new_device_name, const char* new_device_password, JsonDocument& jsonDocument ) {
  ...
  if ( deviceCredentialsChanged ) {
    MYDEBUG_PRINT("Nouveau Nom d'appareil : ");
    MYDEBUG_PRINT(device_name);
    MYDEBUG_PRINT(" | Nouveau Mot de Passe : ");
    MYDEBUG_PRINTLN(device_password);
...
void update_network_config( const char* new_net_ssid, const char* new_net_password, JsonDocument& jsonDocument ) {
  ...
  if ( networkCredentialsChanged ) {
    MYDEBUG_PRINT("Nouveau SSID : ");
    MYDEBUG_PRINT(net_ssid);
    MYDEBUG_PRINT(" | Nouveau Mot de Passe Réseau : ");
    MYDEBUG_PRINTLN(net_password);
 ...
 */
void update_monitoring_config( float new_time, float new_distance, float new_temperature, JsonDocument& jsonDocument ) {
  ...
  if ( temperature_threshold != new_temperature ) {
    monitoringConfigChanged = true;
  }
  temperature_threshold = new_temperature;
  jsonDocument["monitoring"]["temperature"] = temperature_threshold;

  if ( monitoringConfigChanged ) {
    MYDEBUG_PRINT("Nouveau temps de surveillance: ");
    MYDEBUG_PRINT(monitoring_time);
    MYDEBUG_PRINT(" | Nouvelle distance de surveillance: ");
    MYDEBUG_PRINT(monitoring_distance);
    MYDEBUG_PRINT(" | Nouvelle Temperature Seuil: ");
    MYDEBUG_PRINTLN(temperature_threshold);
    InvokeMonitoringConfigChangedCallbacks(monitoring_time, monitoring_distance, temperature_threshold);
  }
 ...
 * @param new_distance La nouvelle distance de monitoring.
 */
void update_monitoring_config( float new_time, float new_distance, float new_temperature ) {

  read_write_file(CONFIG_FILE_PATH, [new_time, new_distance, new_temperature](JsonDocument& jsonDocument, File& jsonFile) {

    update_monitoring_config(new_time, new_distance, new_temperature, jsonDocument);
...
void update_health_status( HealthStatus new_health_status, JsonDocument& jsonDocument ) {
  ...
  if ( healthStatusChanged ) {
    MYDEBUG_PRINT("Nouveau Status de Santé: ");
    MYDEBUG_PRINTLN(user_status == HEALTHY ? "Sain" : user_status == CONTACT ? "Cas Contact" : "Malade");
...
void get_config() {
  ...
  read_write_file(CONFIG_FILE_PATH, [](JsonDocument& jsonDocument, File& jsonFile) {
    ...
    auto json_temperature_threshold = jsonDocument["monitoring"]["temperature"];
    if ( json_temperature_threshold.isNull() ) {
      MYDEBUG_PRINTLN("-SPIFFS : Pas de Seuil de Temperature dans le fichier de configuration");
      json_temperature_threshold = DEFAULT_TEMPERATURE_THRESHOLD;
    }


    update_monitoring_config(json_monitoring_time.as<float>(), json_monitoring_distance.as<float>(), json_temperature_threshold.as<float>(), jsonDocument);
